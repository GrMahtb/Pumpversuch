'use strict';
console.log('HTB Pumpversuch app.js v31 loaded');

const BASE = '/Pumpversuch/';
const STORAGE_DRAFT   = 'htb-pumpversuch-draft-v14';
const STORAGE_HISTORY = 'htb-pumpversuch-history-v14';
...
const state = {
  meta: { objekt:'', grundstueck:'', ort:'', geologie:'', auftragsnummer:'', bauleitung:'', bohrmeister:'', koordination:'', geprueftDurch:'', geprueftAm:'' },
...
function fmtSci(v,digits=2){ const n=Number(v); if(!Number.isFinite(n)||n<=0) return '—'; const [m,e]=n.toExponential(digits).split('e'); return `${m.replace('.',',')}e${Number(e)}`; }
function fmtKf(v){ const n=Number(v); if(!Number.isFinite(n)||n<=0) return '—'; if(n>=0.001) return `${fmtComma(n,6)} m/s`; return `${fmtSci(n,2)} m/s`; }
...
function formatElapsed(ms){ const total=Math.max(0,Math.floor(ms/1000)); const hh=Math.floor(total/3600); const mm=Math.floor((total%3600)/60); const ss=total%60; if(hh>0) return `${hh}:${String(mm).padStart(2,'0')}:${String(ss).padStart(2,'0')}`; return `${String(mm).padStart(2,'0')}:${String(ss).padStart(2,'0')}`; }
...
function m3hToLs(v){ const n=Number(v); return Number.isFinite(n)?(n/3.6).toFixed(3):''; }
function lsToM3h(v){ const n=Number(v); return Number.isFinite(n)?(n*3.6).toFixed(3):''; }
function calcDelta(messwert,ruhe){ const m=Number(messwert),r=Number(ruhe); if(!Number.isFinite(m)||!Number.isFinite(r)||String(messwert).trim()==='') return ''; return (m-r).toFixed(3); }
...
function sortMessungen(v){ v.messungen.sort((a,b)=>{const av=Number(a.min),bv=Number(b.min);const af=Number.isFinite(av),bf=Number.isFinite(bv);if(af&&bf) return av-bv;if(af) return -1;if(bf) return 1;return 0;}); syncIntervalleStrFromRows(v); }
...
function getAverageFoerderMengeNumber(v){ const values=(v.messungen||[]).filter(m=>String(m.foerder_menge??'').trim()!==''&&Number.isFinite(Number(m.foerder_menge))).map(m=>Number(m.foerder_menge)); if(!values.length) return NaN; return values.reduce((s,n)=>s+n,0)/values.length; }
...
/* ───────── audio alarm (lauter, länger, robust für iOS/Android) ───────── */
...
function unlockAlarmAudio(){
  ...
  try{
    ...
    // iOS-Trick: stilles Buffer in derselben User-Geste abspielen
...
function playLayeredTone(ctx,{start,duration=0.4,freq=880,volume=0.45}){
  // Sinus + leiser Square-Layer = lauter wahrgenommen, aber kein Übersteuern
  ...
  const make=(type,vol)=>{
    ...
    g.gain.exponentialRampToValueAtTime(Math.max(0.0001,vol),start+0.012);
    g.gain.setValueAtTime(Math.max(0.0001,vol),start+duration-0.05);
    g.gain.exponentialRampToValueAtTime(0.0001,start+duration);
    osc.connect(g); g.connect(out);
    osc.start(start); osc.stop(start+duration+0.05);
  };
  make('sine',  volume);
  make('square',volume*0.35);
}
function playIntervalBeep(){
  // Vibration (Android, manche Browser) parallel
  try{ navigator.vibrate?.([350,120,350,120,800]); }catch{}
  const ctx=getAlarmAudioContext();
  if(!ctx) return false;
  try{ if(ctx.state==='suspended') ctx.resume(); }catch{}
  if(ctx.state==='suspended') return false;
  const now=ctx.currentTime+0.02;
  // 3-Phasen-Alarm, ~1.8s, deutlich lauter
  playLayeredTone(ctx,{ start: now+0.00, duration:0.40, freq: 880,  volume:0.55 });
  playLayeredTone(ctx,{ start: now+0.55, duration:0.40, freq: 988,  volume:0.55 });
  playLayeredTone(ctx,{ start: now+1.10, duration:0.75, freq: 1320, volume:0.65 });
  return true;
}
...
/* ───────── Kf / Diagramm helpers ───────── */
function getDisplacementCm(raw,ruhe){ const m=Number(raw),r=Number(ruhe); if(!Number.isFinite(m)||!Number.isFinite(r)||String(raw??'').trim()==='') return NaN; return Math.abs((m-r)*100); }
...
function getWellChartPoints(versuch,key,brunnen){ const field=key==='foerder'?'foerder_m':'schluck_m'; const ruhe=Number(brunnen?.ruhe); return getRowsForExport(versuch).map(row=>({x:Number(row.min),y:getDisplacementCm(row[field],ruhe)})).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y)).sort((a,b)=>a.x-b.x); }
function niceNum(range,round){ if(!Number.isFinite(range)||range<=0) return 1; const exp=Math.floor(Math.log10(range)); const f=range/Math.pow(10,exp); let nf; if(round){ if(f<1.5)nf=1; else if(f<3)nf=2; else if(f<7)nf=5; else nf=10; } else { if(f<=1)nf=1; else if(f<=2)nf=2; else if(f<=5)nf=5; else nf=10; } return nf*Math.pow(10,exp); }
function getNiceAxis(minVal,maxVal,ticks=6){ let min=Number.isFinite(minVal)?minVal:0; let max=Number.isFinite(maxVal)?maxVal:1; if(min===max){ if(min===0) max=1; else { min=Math.min(0,min); max=max*1.1; } } const range=niceNum(max-min,false); const step=niceNum(range/Math.max(2,ticks-1),true); const niceMin=Math.floor(min/step)*step; const niceMax=Math.ceil(max/step)*step; return { min:niceMin, max:niceMax, step }; }
function buildTicks(axis){ const ticks=[]; for(let v=axis.min; v<=axis.max+axis.step/2; v+=axis.step) ticks.push(Number(v.toFixed(10))); return ticks; }
function fmtAxisTick(v,decimals=0){ if(!Number.isFinite(v)) return '—'; return String(Number(v.toFixed(decimals))).replace('.',','); }
...
function getStageKfEstimate(versuch,key,brunnen){
  ...
  if(!Number.isFinite(rateM3h)||rateM3h<=0) return { kf:NaN, used:0, total:0, reason:'Keine gültige Förderrate vorhanden' };
  ...
  const logMean=Math.exp(tail.reduce((sum,it,idx)=>sum+Math.log(it.kf)*weights[idx],0)/sumW);
...
/* ───────── photo handling ───────── */
function downscaleImageFile(file,maxDim=1600,quality=0.78){
...
function syncBrunnenToUi(){ BRUNNEN_FIELDS.forEach(([id,group,key])=>{ const el=$(id); if(el) el.value=state[group][key]||''; }); }
...
function collectBrunnenFromUi(){ BRUNNEN_FIELDS.forEach(([id,group,key])=>{ const el=$(id); if(el) state[group][key]=el.value||''; }); }
...
function collectSnapshot(){
  ...
  return { v:14, meta:clone(state.meta), selection:clone(state.selection), foerder:clone(state.foerder), schluck:clone(state.schluck), versuche:clone(state.versuche) };
}
...
function loadDraft(){ try{ const raw=localStorage.getItem(STORAGE_DRAFT); if(raw) applySnapshot(JSON.parse(raw),true); }catch{} }
...
function buildTableHeadHtml(){
  ...
  if(sel.foerder) html+=`<th class="th-foerder">${getWellLabel('foerder')} m ab OK</th>`;
  if(sel.schluck) html+=`<th class="th-schluck">${getWellLabel('schluck')} m ab OK</th>`;
...
function buildTableRowHtml(v,row,rowIdx){
  ...
  html+=`<td><input class="mess-input menge-input" data-role="foerder-menge" data-row="${rowIdx}" type="number" step="0.001" inputmode="decimal" value="${h(row.foerder_menge)}" /></td>`;
  html+=`</tr>`;
  return html;
}
...
<details class="card card--collapsible versuch-card" data-vid="${h(v.id)}" open>
  <summary class="card__title">
    ...
    <span class="versuch-head-actions">
      <button class="photo-btn ${hasPhoto?'photo-btn--has':''}" data-role="photo-btn" type="button" title="${hasPhoto?'Foto neu aufnehmen':'Beweisfoto aufnehmen'}" aria-label="Foto">📷</button>
      ${hasPhoto?`<button class="photo-del-btn" data-role="photo-del" type="button" title="Foto entfernen" aria-label="Foto entfernen">✕</button>`:''}
  ...
  <div class="card__body versuch-body">
    ${hasPhoto?`<div class="photo-thumb-wrap">
      <div>
        <img class="photo-thumb" src="${h(v.photoDataUrl)}" alt="Beweisfoto Durchflussmesser" />
        <div class="photo-thumb-caption">Beweisfoto Durchflussmesser (wird im PDF vor der Stufe eingefügt)</div>
      </div>
    </div>`:''}
    <div class="versuch-row">
      ...
      <input class="rate-input rate-input--readonly" data-role="avg-foerder-menge" type="text" value="${h(avgFoerderMenge||'—')}" readonly />
    </div>
    <div class="versuch-row">
...
function updateTimerUi(card,versuch){
  ...
  const vid=versuch.id; const t=ensureTimer(vid,versuch);
  const elapsedMs=getElapsedMs(vid,versuch); versuch.elapsedMs=elapsedMs;
  ...
  if(nextEl) nextEl.textContent = nextIv!==undefined ? `Nächste Messung: ${nextIv} min (in ${Math.max(0,Math.ceil((nextIv*60000-elapsedMs)/1000))}s)` : 'Alle Messintervalle erreicht';
...
function triggerIntervalAlarm(vid){
  ...
  if(timerBox){ timerBox.classList.remove('timer-box--alarm'); void timerBox.offsetWidth; timerBox.classList.add('timer-box--alarm'); }
...
function tickTimer(vid){
  ...
  if(passedCount>t.alarmCount){ t.alarmCount=passedCount; triggerIntervalAlarm(vid); }
...
function hardStopTimer(vid){ const t=timerMap[vid]; if(!t) return; try{ if(t.raf) cancelAnimationFrame(t.raf); }catch{} delete timerMap[vid]; }
...
function buildLiveChartSvg(points,key){
  ...
  const gridY=yTicks.map(v=>`<line x1="${ml}" y1="${ty(v)}" x2="${W-mr}" y2="${ty(v)}" stroke="rgba(255,255,255,.12)" stroke-width="1" /><text x="${ml-8}" y="${ty(v)+4}" text-anchor="end" fill="rgba(220,240,255,.75)" font-size="11">${h(fmtAxisTick(v,0))}</text>`).join('');
  const gridX=xTicks.map(v=>`<line x1="${tx(v)}" y1="${mt}" x2="${tx(v)}" y2="${mt+ph}" stroke="rgba(255,255,255,.08)" stroke-width="1" /><text x="${tx(v)}" y="${H-16}" text-anchor="middle" fill="rgba(220,240,255,.75)" font-size="11">${h(fmtAxisTick(v,0))}</text>`).join('');
  const polyPoints=points.map(p=>`${tx(p.x)},${ty(p.y)}`).join(' ');
  const circles=points.map(p=>`<circle cx="${tx(p.x)}" cy="${ty(p.y)}" r="3.5" fill="${color}" stroke="#ffffff" stroke-width="1.2" />`).join('');
  return `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" aria-label="Diagramm">
...
function buildLiveWellPanelHtml(versuch,idx,key,brunnen){
  ...
  const qualityText=est.quality==='gut'?'stabil':est.quality==='mittel'?'mittel':'vorläufig';
  return `
<section class="live-well ${wellCls}">
  <div class="live-well__head">
    ...
    <div class="kf-box">
      ...
      <div class="kf-box__note">${Number.isFinite(est.kf)?`Basis: ${h(est.rateSource||'Rate')} · ${h(fmtMaybe(est.rateM3h,3))} m³/h · ${h(String(est.used))} Messpunkte`:h(est.reason||'Noch keine Auswertung möglich')}</div>
...
function renderLiveTab(){
  ...
  if(!state.versuche.length){
    host.innerHTML=`<section class="card"><div class="empty-state">Noch keine Pumpstufe vorhanden.<br />Bitte zuerst im Protokoll eine Pumpstufe anlegen.</div></section>`;
    return;
  }
  const sel=getSelectedWells();
...
function hookStaticInputs(){
  META_FIELDS.forEach(([id])=>{ const el=$(id); if(!el) return;
  ...
  BRUNNEN_FIELDS.forEach(([id])=>{ const el=$(id); if(!el) return;
...
async function handlePhotoSelected(versuch, file){
  if(!file) return;
  try{
    const dataUrl = await downscaleImageFile(file, 1600, 0.78);
    versuch.photoDataUrl = dataUrl;
    renderVersuche();
    saveDraftDebounced();
  }catch(err){
    console.error(err);
    alert('Foto konnte nicht verarbeitet werden: '+(err?.message||String(err)));
  }
}

function hookVersuchDelegation(){
  const host=$('versucheContainer'); if(!host||host.dataset.bound==='1') return;
  ...
  host.addEventListener('input',(e)=>{
    ...
    const role=el.dataset.role; const idx=Number(el.dataset.row);
  ...
  host.addEventListener('change',(e)=>{
    ...
    if(role==='photo-input'){
      ...
      if(file) handlePhotoSelected(versuch,file);
      el.value=''; // reset
      return;
    }
    if(role==='intervalle'){
      ...
      hardStopTimer(versuch.id); renderVersuche(); renderLiveTab(); saveDraftDebounced();
      return;
    }
  ...
  host.addEventListener('click',(e)=>{
    ...
    const card=btn.closest('.versuch-card'); if(!card) return;
    const versuch=getVersuchById(card.dataset.vid); if(!versuch) return;
    const role=btn.dataset.role;

    // Foto-Buttons: dürfen <details> nicht togglen
    if(role==='photo-btn'){
      e.preventDefault(); e.stopPropagation();
      unlockAlarmAudio();
      const input=card.querySelector('input[data-role="photo-input"]');
      input?.click();
      return;
    }
    if(role==='photo-del'){
      ...
      renderVersuche(); saveDraftDebounced();
      return;
    }
    ...
    if(role==='row-plus'){
      ...
      renderVersuche(); renderLiveTab(); saveDraftDebounced();
      return;
    }
    if(role==='del'){
...
function buildHistoryKfHtml(snapshot){
  ...
  const lines=versuche.map((raw,idx)=>{
    ...
    const parts=[];
    if(snapshot.selection?.foerder){ const e=getStageKfEstimate(v,'foerder',snapshot.foerder||{}); parts.push(`Entnahme: ${Number.isFinite(e.kf)?fmtKf(e.kf):'—'}`); }
...
function hookHistoryDelegation(){
  ...
  host.addEventListener('click', async (e)=>{
    ...
    const id=btn.dataset.id; const act=btn.dataset.hact;
    ...
    if(act==='pdf'){ try{ await exportPdf(entry.snapshot); }catch(err){ console.error(err); alert('PDF-Fehler: '+(err?.message||String(err))); } }
  });
}
function renderHistoryList(){
  ...
  host.innerHTML=list.map(entry=>{
    ...
    const wells=[];
    if(snap.selection?.foerder) wells.push('Entnahmebrunnen');
    if(snap.selection?.schluck) wells.push('Rückgabebrunnen');
...
function getWellRowsForPdf(versuch,key,ruhe){
  ...
  return getRowsForExport(versuch).map(r=>{
    const min=Number(r.min); const raw=r[field];
...
function drawWellTable(page,opt){
  const { x,yTop,w,key,rows,ruhe,fontR,fontB,K,grey }=opt;
  const title=getWellLabel(key);
...
function drawWellChart(page,opt){
  ...
  const title='Diagramm '+getWellLabel(key);
  ...
  drawTextSafe(page,title,{ x:x+4, y:y+h-9, size:7.6, font:fontB, color:K });
  ...
  yTicks.forEach(v=>{ const yy=ty(v); page.drawLine({ start:{x:px,y:yy}, end:{x:px+pw,y:yy}, thickness:0.5, color:gridColor }); drawTextSafe(page, fmtAxisTick(v,0), { x:px-22, y:yy-2, size:6.2, font:fontR, color:K }); });
  xTicks.forEach(v=>{ const xx=tx(v); page.drawLine({ start:{x:xx,y:py}, end:{x:xx,y:py+ph}, thickness:0.5, color:gridColor }); drawTextSafe(page, fmtAxisTick(v,0), { x:xx-6, y:plotTop+4, size:6.2, font:fontR, color:K }); });
...
function drawStageSplitLayout(page,opt){
  const { x,yTop,yBottom,w,versuch,foerder,schluck,fontR,fontB,K,grey,degrees,rgb,selection={foerder:true,schluck:true} }=opt;
  const stageH=22;
  page.drawRectangle({ x, y:yTop-stageH, width:w, height:stageH, color:grey, borderColor:K, borderWidth:0.8 });
  const rateLs=getPdfRateLs(versuch); const rateM3h=getPdfRateM3h(versuch);
  drawTextSafe(page, `Pumpversuch   ${versuch._stageTitle||'Stufe'}   ${rateLs} [l/s]`, { x:x+4, y:yTop-stageH+11, size:8.5, font:fontB, color:K });
  drawTextSafe(page, `${rateM3h} [m³/h]`, { x:x+4, y:yTop-stageH+4, size:7.5, font:fontR, color:K });

  const allKeys=['foerder','schluck'];
  const keys=allKeys.filter(k=>selection[k]);
  ...
  keys.forEach((key,i)=>{
    ...
    const tableH=drawWellTable(page,{ x:colX, yTop:tableTop, w:colW, key, rows, ruhe:well?.ruhe, fontR, fontB, K, grey });
...
async function drawPhotoPage(pdf,v,opts){
  const { fontR,fontB,K,GREY,mm,PAGE_W,PAGE_H,logo,stageTitle,meta }=opts;
  const page=pdf.addPage([PAGE_W,PAGE_H]);
  const margin=mm(8); const x0=margin, y0=margin;
  const W=PAGE_W-2*margin, H=PAGE_H-2*margin;
  ...
  drawTextSafe(page,'Pumpversuch – Beweisfoto',{ x:x0+mm(32), y:y0+H-hdrH+mm(4.2), size:13, font:fontB, color:K });
  drawTextSafe(page,`${stageTitle} · Durchflussmesser${meta?.objekt?` · ${meta.objekt}`:''}`,{ x:x0+mm(32), y:y0+H-hdrH+mm(1.5), size:8, font:fontR, color:K });

  // Foto einbetten
  try{
    const dataUrl=v.photoDataUrl||'';
    ...
    const padBottom=mm(14);
    const areaY=y0+padBottom;
    const areaH=H-hdrH-padBottom-mm(4);
    const areaX=x0+mm(4);
    const areaW=W-mm(8);
    ...
    console.error('Foto-Einbettung fehlgeschlagen',e);
    drawTextSafe(page,'Foto konnte nicht eingebettet werden.',{ x:x0+20, y:y0+H/2, size:10, font:fontR, color:K });
  }
}
...
async function exportPdf(snapshot=null){
  ...
  const fontBytesR=await fetch(`${BASE}fonts/arial.ttf?v=60`).then(r=>{ if(!r.ok) throw new Error('arial.ttf nicht gefunden'); return r.arrayBuffer(); });
  let fontBytesB=null;
  try{ fontBytesB=await fetch(`${BASE}fonts/arialbd.ttf?v=60`).then(r=>{ if(!r.ok) throw new Error('arialbd.ttf nicht gefunden'); return r.arrayBuffer(); }); }catch{}
  ...
  let logo=null;
  try{
    const bytes=await fetch(`${BASE}logo.png?v=30`).then(r=>{ if(!r.ok) throw new Error(r.status); return r.arrayBuffer(); });
    logo=await pdf.embedPng(bytes);
  }catch{}

  const PAGE_W=595.28, PAGE_H=841.89;
  const mm=v=>v*72/25.4;
  const K=rgb(0,0,0); const GREY=rgb(0.90,0.90,0.90);

  const meta=snap.meta||{};
  const foerder=snap.foerder||{};
  const schluck=snap.schluck||{};
  const selection = snap.selection || { foerder:true, schluck:true };
  if(!selection.foerder && !selection.schluck){ selection.foerder=true; }

  for(let i=0;i<versuche.length;i++){
    const v=hydrateVersuch(versuche[i]);
    v._stageTitle=getStageTitle(i);

    // 1) Beweisfoto-Seite (vorab) – falls vorhanden
    if(v.photoDataUrl){
      await drawPhotoPage(pdf, v, { fontR, fontB, K, GREY, mm, PAGE_W, PAGE_H, logo, stageTitle:v._stageTitle, meta });
    }
    ...
    page.drawRectangle({ x:x0, y:y0+H-hdrH, width:W, height:hdrH, color:GREY, borderColor:K, borderWidth:0.8 });
    if(logo){
      const lh=hdrH*0.75; const scale=lh/logo.height;
      page.drawImage(logo,{ x:x0+mm(2), y:y0+H-hdrH+(hdrH-lh)/2, width:logo.width*scale, height:lh });
    }
    ...
    // Ruhewasserspiegel (dynamisch je nach Auswahl)
    ...
    const wellsRW=[];
    if(selection.foerder) wellsRW.push({ label:`${getWellLabel('foerder')} ab OK Brunnenausbau`, value: foerder.ruhe?fmtComma(foerder.ruhe,3):'—' });
    if(selection.schluck) wellsRW.push({ label:`${getWellLabel('schluck')} ab OK Brunnenausbau`, value: schluck.ruhe?fmtComma(schluck.ruhe,3):'—' });
    ...
    const wellTexts=[];
    if(selection.foerder) wellTexts.push(`${getWellLabel('foerder')}: Ø ${foerder.dm||'—'} mm · ET ${foerder.endteufe||'—'} m`);
    if(selection.schluck) wellTexts.push(`${getWellLabel('schluck')}: Ø ${schluck.dm||'—'} mm · ET ${schluck.endteufe||'—'} m`);
    ...
    drawStageSplitLayout(page,{
      ...
      versuch:v, foerder, schluck,
      fontR, fontB, K, grey:GREY, degrees, rgb,
      selection
    });

    page.drawLine({ start:{x:x0,y:y0+mm(5.5)}, end:{x:x0+W,y:y0+mm(5.5)}, thickness:0.8, color:K });
    drawTextSafe(page,`Seite ${i+1}/${versuche.length}`,{ x:x0+W-40, y:y0+4, size:7, font:fontR, color:K });
  }
...
function resetAll(){
  ...
  Object.keys(timerMap).forEach(hardStopTimer);
  state.meta={ objekt:'', grundstueck:'', ort:'', geologie:'', auftragsnummer:'', bauleitung:'', bohrmeister:'', koordination:'', geprueftDurch:'', geprueftAm:'' };
  state.selection={ foerder:true, schluck:true };
  state.foerder={ dm:'', endteufe:'', ruhe:'' };
  state.schluck={ dm:'', endteufe:'', ruhe:'' };
  state.versuche=[];
  syncMetaToUi(); syncBrunnenToUi(); syncSelectionToUi(); renderVersuche(); renderLiveTab(); saveDraftDebounced();
}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  $('btnSave')?.addEventListener('click',()=>{ saveCurrentToHistory(); saveDraftDebounced(); alert('Pumpversuch im Verlauf gespeichert.'); });
  $('btnPdf')?.addEventListener('click', async ()=>{ try{ await exportPdf(); }catch(err){ console.error(err); alert('PDF-Fehler: '+(err?.message||String(err))); } });
  $('btnReset')?.addEventListener('click',resetAll);

  if('serviceWorker' in navigator){
    navigator.serviceWorker.register(`${BASE}sw.js?v=31`).catch(err=>console.error('SW:',err));
  }
});
