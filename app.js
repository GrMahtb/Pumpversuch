...
/* ════════════════════════════════════════════════════════════════
   §2  DOM-HILFSFUNKTION
   $() als Kurzform für document.getElementById()
   ════════════════════════════════════════════════════════════════ */
const $=id=>document.getElementById(id);

/* ════════════════════════════════════════════════════════════════
   §3  STATE – ZUSTANDSOBJEKT
   getInitialState() definiert die vollständige Datenstruktur.
   state ist das globale Singleton zur Laufzeit.
   timerMap und Debounce-Handles sind ebenfalls global.
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Manuelle Rate, Ø Fördermenge, Dupuit-Iteration (30 Schritte),
   Kf-Qualitätsbewertung (gut/mittel/gering), Chart-Datenpunkte
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   §7  FELDZUORDNUNG
   META_FIELDS und BRUNNEN_FIELDS: DOM-ID → State-Key Mapping
   ════════════════════════════════════════════════════════════════ */
const META_FIELDS=[
...
/* ════════════════════════════════════════════════════════════════
   ...
   syncXxxToUi()      State → DOM
   collectXxxFromUi() DOM → State
   renderXxx()        DOM dynamisch neu aufbauen
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   LocalStorage (Draft-Autosave), IndexedDB (History),
   Foto-Blob-Handling, History-CRUD,
...
/* ════════════════════════════════════════════════════════════════
   §10 TABS – NAVIGATION
   Aktiviert/deaktiviert Panes und triggert Tab-spezifisches Rendering
   ════════════════════════════════════════════════════════════════ */
function initTabs(){
...
/* ════════════════════════════════════════════════════════════════
   §11 AUDIO & ALARM
   Web Audio API, iOS AudioContext-Unlock, Intervall-Beep,
   Vibration (navigator.vibrate), Alarm-Button-Status
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Downscale (max. 1400 px, JPEG 0.74), DataUrl ↔ Uint8Array/Blob,
   embedDataUrlImage() für PDF-Einbettung
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   §13 FOTO-DELEGATION
   Zentraler Click/Change-Handler für alle Foto-Buttons und
   Datei-Inputs (Übersicht, Restsand, pH, Versuch-Beweisfoto)
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   §14 TIMER-STEUERUNG
   timerMap pro Versuch: { running, startMs, accumulatedMs, raf }
   start / stop / reset / hardStop, Intervall-Alarm-Triggering
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Fixiertes Overlay mit aktiver Laufzeit – erscheint wenn der
   Timer-Block durch Scrollen außerhalb des Sichtbereichs ist
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Nachträgliches Vor-/Zurückspulen der Timer-Zeit per Offset [s]
...
/* ════════════════════════════════════════════════════════════════
   ...
   Erzeugt das komplette Pumpstufen-Formular inkl. Messtabelle
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Alle Interaktionen innerhalb einer Versuchskarte:
   Eingaben, Fotos, Timer-Buttons, Intervall-Änderungen, Löschen
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Meta, Brunnen, Restsand, pH, Kolben, Settings,
   alle Action-Buttons (Speichern, PDF, Reset, Export, Import)
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   Vorlage-Export (ohne Messwerte/Fotos, .htbpump.json) und
   Vollexport/-import inkl. Fotos (Base64 DataURL)
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   §21 LIVE-AUSWERTUNG
   SVG-Absenkungsdiagramm (NiceNum-Achsen), Kf-Panel pro Brunnen,
   Live-Rendering pro Pumpstufe (scheduleLiveRender, Debounce 90 ms)
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   §22 VERLAUF (HISTORY)
   renderHistoryList(), hookHistoryDelegation()
   Laden, alle PDF-Typen, Foto-Export, Löschen aus IndexedDB
   ════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════
   ...
   loadPdfAssets (Font, Logo, Fußzeilenbild), getPdfCtx,
   drawFooter, drawHeaderBar, drawMetaGrid,
...
async function loadPdfAssets(pdf){
  ...
  const fontBytesR = await fetch(`${BASE}fonts/LiberationSans-Regular.ttf?v=60`).then(r=>r.arrayBuffer());
  let fontBytesB = null;
  try{ fontBytesB = await fetch(`${BASE}fonts/LiberationSans-Bold.ttf?v=60`).then(r=>r.arrayBuffer()); }catch{}
...
/* ════════════════════════════════════════════════════════════════
   §24 PDF – SEITEN
   drawNewFooterFull, drawCoverPage (Deckblatt), drawTocPage (TOC),
...
/* ════════════════════════════════════════════════════════════════
   §25 PDF – EXPORTS
   addFullPdfPageNumbers (ab Seite 3),
   exportPdf (Protokoll / Vollständiger Bericht),
...
/* ════════════════════════════════════════════════════════════════
   ...
   resetAll() setzt alle State-Felder auf Initialwerte zurück
   initInstallButton() steuert den PWA-Installations-Prompt
   ════════════════════════════════════════════════════════════════ */
function resetAll(){
...
/* ════════════════════════════════════════════════════════════════
   ...
   Hooks installieren, Draft laden, UI aufbauen,
